...
struct MarkdownBuilder {
    ...
    func parseComponent(component: ArticleComponent, font: UIFont) -> NSMutableAttributedString? {
        ...
        retval.addAttribute(NSFontAttributeName, value: font, range: NSMakeRange(0, component.text.characters.count))
        switch type {
        case .heading:
            let block = MarkdownHeader(meta: [], text: component.text, font: font, depth: component.depth).build()
            block.enumerateAttributes(in: NSMakeRange(0, component.text.characters.count), options: [], using: { (attributes, range, pointer) in
        ...
        case .paragraph:
            ...
            paragraphAttributes.enumerateAttributes(in: NSMakeRange(0, component.text.characters.count), options: [], using: { (attributes, range, pointer) in
                retval.addAttributes(attributes, range: range)
            })
            return retval
        case .image:
